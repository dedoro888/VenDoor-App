              ...
              <div className="relative aspect-[9/12] bg-secondary">
                ...
                <div className="absolute right-3 bottom-20 flex flex-col items-center gap-5">
                  <button onClick={() => toggleLike(video.id)} className="flex flex-col items-center gap-1">
                    <div className={`w-10 h-10 rounded-full backdrop-blur-sm flex items-center justify-center transition-colors ${liked.has(video.id) ? "bg-destructive/30" : "bg-background/20"}`}>
                      <Heart className={`w-5 h-5 ${liked.has(video.id) ? "fill-destructive text-destructive" : "text-white"}`} />
                    </div>
                  ...
                  <button onClick={() => handleAddToOrders(video.foodId)} className="flex flex-col items-center gap-1">
